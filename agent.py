...
# ============================================================
# CRITICAL: Set cache directories to E: drive BEFORE any imports
# ============================================================
...
# Set environment variables for ChromaDB, HuggingFace, ONNX, and embedding models
# This MUST be done before importing chromadb or any embedding models
...
os.environ["ONNX_RUNTIME_CACHE_DIR"] = str(cache_dir / "onnx_models")

# Disable HuggingFace symlinks warning on Windows
os.environ["HF_HUB_DISABLE_SYMLINKS_WARNING"] = "1"

# Ensure no caching on C drive
c_drive_paths = [
    "C:\\Users\\Pc Planet\\.cache\\chroma",
    "C:\\Users\\Pc Planet\\.cache\\huggingface",
    "C:\\Users\\Pc Planet\\.cache\\transformers",
]
...
for path in c_drive_paths:
    ...
    if cache_path.exists():
        try:
            ...
            print(f"[INFO] Cleared C drive cache: {path}")
        except Exception as e:
            print(f"[WARN] Could not clear {path}: {e}")

print(f"[INFO] Cache configured to use E drive: {cache_dir}")
print(f"[INFO] CHROMA_CACHE_DIR={os.environ.get('CHROMA_CACHE_DIR')}")
print(f"[INFO] HF_HOME={os.environ.get('HF_HOME')}")
print(f"[INFO] ONNX_RUNTIME_CACHE_DIR={os.environ.get('ONNX_RUNTIME_CACHE_DIR')}")
import json
import torch
import httpx
...
if __name__ == "__main__":
    res3 = process_query("Get me the monthly summary of last month")
    print("\nResult:", res3)
    res4 = process_query("Summarise what you know about me and the company till now?")
    print("\nResult:", res4)
    import os
import sys
import shutil
...
# Token-based conversation buffer — no max_size, budget-driven eviction
...
def process_query(user_query: str):
    ...
    print(f"Matched Tool: {selected_tool['name']} (Confidence: {best_score:.4f})")
    ...
    try:
        ...
        # Log token budget after every query
        _log_budget()
...
def _log_budget():
    used = conversation_buffer.get_total_tokens()
    budget = conversation_buffer.budget
    remaining = conversation_buffer.get_budget_remaining()
    print(f"\n[TOKEN BUDGET] Used: {used:,} / {budget:,} | Remaining: {remaining:,}")
    print(f"[TOKEN BUDGET] Slots in buffer: {len(conversation_buffer.turns)} turns")
...
if __name__ == "__main__":
    queries = [
        "Who owes us money and how overdue are they?",           # aging report
        "Give me last month's income and expense summary",        # monthly summary
        "Are our books balanced? Run a reconciliation",           # reconciliation
        "Any unusual expenses I should know about?",              # anomaly detection                   # should hit confidence guardrail
    ]
